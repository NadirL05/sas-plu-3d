...
function StaticFallback({ hint }: { hint?: string }) {
  return (
    <div className="relative h-full w-full overflow-hidden rounded-2xl border border-white/10 bg-slate-950/80">
      ...
      />
      {hint ? (
        <div className="absolute inset-x-0 bottom-0 bg-black/40 px-4 py-2 text-center text-xs text-slate-200 backdrop-blur-sm">
          {hint}
        </div>
      ) : null}
    </div>
...
export function SplineHero() {
  ...
  useEffect(() => {
    if (isMobile || isSceneReady || hasRuntimeError) return;
  ...
  }, [hasRuntimeError, isMobile, isSceneReady]);
  ...
  if (isMobile || hasRuntimeError) {
    return <StaticFallback hint={hasRuntimeError ? "Aperçu simplifié chargé" : undefined} />;
  }

  return (
    <SplineErrorBoundary
      onError={() => setHasRuntimeError(true)}
      fallback={<StaticFallback hint="Aperçu simplifié chargé" />}
    >
